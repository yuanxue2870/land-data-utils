...
! Define 2D array 
  real   ,  allocatable :: era5SnowD(:,:)
  real   ,  allocatable :: era5SnowWE(:,:)  
...
! read input file for dates
  ...
  allocate(era5SnowD(destination_lons, destination_lats))
  allocate(era5SnowWE(destination_lons, destination_lats))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  source_filename=trim(source_path)//'ERA5_snow_t2m_00Z_1Jan_31May26.nc'
...
! read snow density
  ...
  destination_filename=trim(destination_path)//trim(fv3_grid)//'_era5_snow_'//trim(idate(id))//'00.nc'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i = 1, destination_lons
    itotal=itotal+1
    era5SnowWE(i,j) = era5SWE(itotal)
    era5SnowD(i,j) =  era5SnowDensity(itotal)
...
! Define variables in the file.
  ...
  status = nf90_def_var(ncid, "SWE", NF90_FLOAT, (/dim_id_i, dim_id_j, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "long_name", "era5 snow water equivalent")
    if (status /= nf90_noerr) call handle_err(status)
  ...
  status = nf90_put_att(ncid, varid, "_FillValue", fillVal)
   ...
   status = nf90_def_var(ncid, "SnowDensity", NF90_FLOAT, (/dim_id_i, dim_id_j, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "long_name", "era5 snow density")
    if (status /= nf90_noerr) call handle_err(status)
...
! Write variables in the file.
  ...
  status = nf90_inq_varid(ncid, "SWE", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid , era5SnowWE, start = (/1,1,1/), count = (/destination_lons, destination_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "_FillValue", fillVal)
    if (status /= nf90_noerr) call handle_err(status)
  ...
  status = nf90_inq_varid(ncid, "SnowDensity", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid , era5SnowD, start = (/1,1,1/), count = (/destination_lons, destination_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "_FillValue", fillVal)
    if (status /= nf90_noerr) call handle_err(status)
