program data_regrid_gaussian
  ...
  integer, parameter           :: source_lats           = 721
...
! Define 1D lat and lon
  ...
  namelist/regrid_era5_nml/ destination_locs, weight_locs, destination_lons, destination_lats, fv3_grid, source_path, weights_filename, destination_path

! read namelist

  inquire(file='regrid_era5.nml', exist=file_exists)
  ...
  open (action='read', file='regrid_era5.nml', iostat=ierr, newunit=io)
...
! read input file for dates
  ...
  allocate(era5t(destination_locs))
  allocate(era5tWT(destination_locs))
  allocate(xcb(destination_locs))
  allocate(ycb(destination_locs))

  allocate(era5t2m(destination_lons, destination_lats))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  source_filename=trim(source_path)//'ERA5_snow_t2m_00Z_1Jan_31May25.nc'
!  write(*,*) trim(source_filename)

  status = nf90_open(trim(source_filename), NF90_NOWRITE, ncid)
    if (status /= nf90_noerr) call handle_err(status)

! read 2m air temperature
  status = nf90_inq_varid(ncid, "t2m", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_get_var(ncid, varid , t2m)
    if (status /= nf90_noerr) call handle_err(status)
  ...
  destination_filename=trim(destination_path)//trim(fv3_grid)//'_era5_t2m_'//trim(idate(id))//'00.nc'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do iwt = 1, weight_locs
    ...
    if(source_input(lonloc,latloc) >= 0.0 ) then
      era5t(destination_lookup(iwt)) = era5t(destination_lookup(iwt)) + weights(iwt) * source_input(lonloc,latloc)
      era5tWT(destination_lookup(iwt)) = era5tWT(destination_lookup(iwt)) + weights(iwt)
    end if

  end do
  do iwt = 1, destination_locs

    if(era5tWT(iwt) > 0.0 ) then
     era5t(iwt) = era5t(iwt)/era5tWT(iwt)
    else
     era5t(iwt) = -9999.0
    end if
...
! Define variables in the file.
  ...
  status = nf90_def_var(ncid, "ERA5_t2m", NF90_FLOAT, (/dim_id_i, dim_id_j, dim_id_t/), varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "long_name", "era5 2m air temperature")
    if (status /= nf90_noerr) call handle_err(status)
...
! Write variables in the file.
  ...
  status = nf90_inq_varid(ncid, "ERA5_t2m", varid)
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_var(ncid, varid ,era5t2m, start = (/1,1,1/), count = (/destination_lons, destination_lats,1/))
    if (status /= nf90_noerr) call handle_err(status)
  status = nf90_put_att(ncid, varid, "_FillValue", fillVal)
    if (status /= nf90_noerr) call handle_err(status)
